...
void VideoCapture::AddFrame(uint8_t *data) {
    ...
    //AV_PIX_FMT_GRAY8
    if (!swsCtx) {
        swsCtx = sws_getContext(cctx->width, cctx->height, AV_PIX_FMT_GRAY8, cctx->width, cctx->height, AV_PIX_FMT_YUV420P, SWS_BICUBIC, 0, 0, 0);
    }
...
void VideoCapture::Remux(const char* out_file_name) {
    ...
    if ((err = avformat_alloc_output_context2(&ofmt_ctx, NULL, NULL, out_file_name))) {
        Debug("Failed to allocate output context", err);
        goto end;
    }
    {
        ...
        if (!(ofmt_ctx->oformat->flags & AVFMT_NOFILE)) {
            if ((err = avio_open(&ofmt_ctx->pb, out_file_name, AVIO_FLAG_WRITE)) < 0) {
