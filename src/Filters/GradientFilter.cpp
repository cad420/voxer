...
vector<uint8_t> GradientFilter::process(StructuredGrid &data) {
  ...
  auto getValue = [&buffer, &dimension, offset_z](int x, int y, int z) {
    if (x < 0 || y < 0 || z < 0 || x >= dimension[0] || y >= dimension[1] ||
        z >= dimension[2]) {
      return static_cast<uint8_t>(0);
    }

    int index = z * offset_z + y * dimension[0] + x;
    return buffer[index];
  };

  auto calGradient = [buffer, data, getValue](int px, int py, int pz) {
    uint8_t t0[3][3][3];
    float t1[3][3];
    float t2[3];
  ...
  result.resize(dimension[0] * dimension[1] * dimension[2] * 4, 0);
