...
//layout(binding=0,rgba32f) uniform image2D entryPosTexture;
//layout(binding=1,rgba32f) uniform image2D exitPosTexture;
...
vec4 calColor()
{
    vec3 startPos=texture(entryPosTexture,vec2(float(gl_FragCoord.x)/1200.0,float(gl_FragCoord.y)/900.0)).xyz;
    vec3 exitPos=texture(exitPosTexture,vec2(float(gl_FragCoord.x)/1200.0,float(gl_FragCoord.y)/900.0)).xyz;
