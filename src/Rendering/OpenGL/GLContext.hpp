...
enum EventAction { Press = 0x01, Release = 0x02, Repeat = 0x03, Move = 0x04 };
enum MouseButton { Mouse_Left = 0x01, Mouse_Right = 0x02 };
enum KeyButton {
  Key_Unknown,
  Key_0,
  Key_1,
...
using MouseEventCallback = std::function<void(
    void *, MouseButton buttons, EventAction action, int xpos, int ypos)>;
using ScrollEventCallback =
    std::function<void(void *, int xoffset, int yoffset)>;
using KeyboradEventCallback =
    std::function<void(void *, KeyButton key, EventAction action)>;
using FileDropEventCallback =
    std::function<void(void *, int count, const char **df)>;
using FramebufferResizeEventCallback =
    std::function<void(void *, int width, int height)>;
