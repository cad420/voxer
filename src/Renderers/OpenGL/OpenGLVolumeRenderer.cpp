...
//    glActiveTexture(GL_TEXTURE0);
    ...
    glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA, tfcn_dim, 0, GL_RGBA, GL_UNSIGNED_BYTE, data.data());
...
unsigned int cube_vertex_indices[] = {0, 1, 2, 0, 2, 3, 0, 4, 1, 4, 5, 1, 1, 5, 6, 6, 2, 1,
                                      6, 7, 2, 7, 3, 2, 7, 4, 3, 3, 4, 0, 4, 7, 6, 4, 6, 5};

float screen_quad_vertices[] = {-1.0f, 1.0f, 0.0f, 1.0f, -1.0f, -1.0f, 0.0f, 0.0f, 1.0f, -1.0f, 1.0f, 0.0f,
...
void OpenGLVolumeRenderer::setup_context()
{
    ...
    m_egl_context = eglCreateContext(display, egl_config, m_cache->get_context(), context_attri);
...
void OpenGLVolumeRenderer::set_camera(const Camera &camera)
{
    ...
    glTextureStorage2D(m_entry_texture, 1, GL_RGBA32F, camera.width, camera.height);
    glBindImageTexture(1, m_entry_texture, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA32F);

    // create texture for ray exit info
    glGenTextures(1, &m_exit_texture);
    glActiveTexture(GL_TEXTURE0 + 5);
    glBindTexture(GL_TEXTURE_2D, m_exit_texture);
    glTextureStorage2D(m_exit_texture, 1, GL_RGBA32F, camera.width, camera.height);
    glBindImageTexture(2, m_exit_texture, 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA32F);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_entry_texture, 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, m_exit_texture, 0);

    auto fbo_status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (fbo_status != GL_FRAMEBUFFER_COMPLETE)
    {
        throw runtime_error("Framebuffer Object is not complete: " + to_string(fbo_status));
    }
...
void OpenGLVolumeRenderer::add_isosurface(const std::shared_ptr<voxer::Isosurface> &isosurface)
{
...
{
    auto camera_to_target = glm::vec3(m_camera.target[0] - m_camera.pos[0], m_camera.target[1] - m_camera.pos[1],
                                      m_camera.target[2] - m_camera.pos[2]);
    auto distance = glm::length(camera_to_target);
    auto aspect = static_cast<float>(m_camera.width) / static_cast<float>(m_camera.height);
    auto projection = glm::identity<glm::mat4>();
    if (m_camera.type == Camera::Type::PERSPECTIVE)
    {
        auto fov = 2 * atan(tan(45.0f * glm::pi<float>() / 180.0f / 2.0f) / m_camera.zoom);
    ...
    else
    {
        ...
        projection = glm::ortho(-half_width, half_width, -half_height, half_height, 1.0f, 8000.0f);
    }

    auto view = glm::lookAt(glm::vec3(m_camera.pos[0], m_camera.pos[1], m_camera.pos[2]),
    ...
    m_raycast_program->setVec3("cameraFront", camera_to_target[0], camera_to_target[1], camera_to_target[2]);
    m_raycast_program->setInt("TF1", 0);
    m_raycast_program->setInt("TF2", 1);
//    m_raycast_program->setInt("preIntTF", 1);
    ...
    m_raycast_program->setVec3("bgColor", m_background[0], m_background[1], m_background[2]);
    m_raycast_program->setBool("usePreIntTF", false);
    m_raycast_program->setBool("isPerspective", m_camera.type == Camera::Type::PERSPECTIVE);
    ...
    if (m_camera.pos[0] <= -0.1 || m_camera.pos[0] >= 1.1 || m_camera.pos[1] <= -0.1 || m_camera.pos[1] >= 1.1 ||
...
//        for (auto &isosurface : m_isosurfaces)
        {
            auto& isosurface=m_isosurfaces[0];
            auto dataset = isosurface->dataset.get();
            auto &dimensions = dataset->info.dimensions;
            auto max = static_cast<float>(std::max(dimensions[0], std::max(dimensions[1], dimensions[2])));

            auto model = glm::identity<glm::mat4>();
            model = glm::scale(model, glm::vec3(dimensions[0], dimensions[1], dimensions[2]));
//            model = glm::translate(model, glm::vec3(-0.5f, -0.5f, -0.5f));
            ...
            m_raycast_program->use();
            m_raycast_program->setBool("is_single_volume",true);
            if(m_isosurfaces.size()>1)
            {
                auto& isosurface1=m_isosurfaces[1];
                auto dataset1 = isosurface1->dataset.get();
                auto dataset_texture1 = m_cache->get(dataset1);
                glActiveTexture(GL_TEXTURE0 + 3);
                glBindTexture(GL_TEXTURE_3D, dataset_texture1);
                m_raycast_program->use();
                m_raycast_program->setFloat("isoValue2", isosurface1->value / 255.0f);
                m_raycast_program->setVec3("isoColor2", isosurface1->color.data[0], isosurface1->color.data[1],
                                           isosurface1->color.data[2]);
                m_raycast_program->setBool("is_single_volume",false);
            }
            ...
            // render pass 3
            m_raycast_program->use();
            m_raycast_program->setFloat("isoValue1", isosurface->value / 255.0f);
            m_raycast_program->setVec3("isoColor1", isosurface->color.data[0], isosurface->color.data[1],
                                       isosurface->color.data[2]);
            m_raycast_program->setMat4("model", model);
            m_raycast_program->setMat4("InverseModel", glm::inverse(model));
            m_raycast_program->setVec3("boundRatio", 1.0f / max * dimensions[0], 1.0f / max * dimensions[1],
                                       1.0f / max * dimensions[2]);
            m_raycast_program->setInt("drawMode", 1);
            m_raycast_program->setVec3("boundRatio", 1.0f / max * dimensions[0], 1.0f / max * dimensions[1],
                                       1.0f / max * dimensions[2]);
            m_raycast_program->setFloat("voxel",1.f/max);
            m_raycast_program->setFloat("step",1.f/max*0.1f);
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
    ...
    if(!m_volumes.empty())
    {
        ...
        GLuint tfcn_texture1=0;
        if(m_volumes.size()>1){
            m_raycast_program->use();
            m_raycast_program->setBool("is_single_volume",false);
            auto& volume1=m_volumes[1];
            auto dataset1 = volume1->dataset.get();

            tfcn_texture1 = create_tfcn_texture(*volume1->tfcn);
            glActiveTexture(GL_TEXTURE0 + 1);
            glBindTexture(GL_TEXTURE_1D, tfcn_texture1);
        ...
        m_raycast_program->setFloat("voxel",1.f/max);
        m_raycast_program->setFloat("step",1.f/max*0.3f);
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glDisable(GL_CULL_FACE);
        glBindVertexArray(m_VAO);
        glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, nullptr);
        ...
        glDeleteTextures(1,&tfcn_texture1);
    }
...
auto OpenGLVolumeRenderer::get_colors() -> const Image &
{
    ...
    m_image.channels = 4;
    m_image.data.resize(m_image.width * m_image.height * 4);

    // read pixels from the default frame buffer
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glReadPixels(0, 0, m_image.width, m_image.height, GL_RGBA, GL_UNSIGNED_BYTE,
...
{
    m_position_program = make_unique<ShaderProgram>(shader::position_v, shader::position_f);
    m_raycast_program = make_unique<ShaderProgram>(shader::raycast_v, shader::raycast_f);
    m_screen_program = make_unique<ShaderProgram>(shader::screen_v, shader::screen_f);
//    m_essposition_program = make_unique<ShaderProgram>(shader::essposition_v, shader::essposition_f);
//    m_essraycast_program = make_unique<ShaderProgram>(shader::essraycast_v, shader::essraycast_f);
