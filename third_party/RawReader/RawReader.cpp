...
RawReader::~RawReader() { fclose(file); }

// Read a region of volume data from the file into the buffer passed. It's
// assumed the buffer passed has enough room. Returns the number voxels read
size_t RawReader::readRegion(const vec3sz &start, const vec3sz &size,
                             unsigned char *buffer) {
  assert(size.x > 0 && size.y > 0 && size.z > 0);

  int64_t startRead =
      (start.x + dimensions.x * (start.y + dimensions.y * start.z)) * voxelSize;
  if (offset != startRead) {
  ...
  // Figure out how to actually read the region since it may not be a full X/Y
  // slice and we'll need to read the portions in X & Y and seek around to skip
  // regions
  size_t read = 0;
  if (convexRead(size)) {
