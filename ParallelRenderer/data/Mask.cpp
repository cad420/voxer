...
string createDatasetByMask(string first, string second, vec2f &rangeFirst, vec2f &rangeSecond) {
  ...
  for (auto i = 0; i < limit; i++) {
    ...
    if (firstData >= rangeFirst.x && firstData <= rangeFirst.y && secondData >= rangeSecond.x && secondData <= rangeSecond.y) {
